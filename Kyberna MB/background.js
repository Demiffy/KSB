...
function determineCssFile(result) {
  ...
  return 'styles.css'; // Default CSS file
}
...
function applyThemeOnTab(tabId, url) {
  if (url.includes("https://sis.ssakhk.cz/")) {
      ...
      console.log(`URL does not match or tab is undefined. No CSS injected.`);
  }
}
...
function reloadTab(tabId) {
  chrome.tabs.reload(tabId, () => {
    ...
    lastReloadTimes[tabId] = Date.now(); // Update last reload time
  });
}
